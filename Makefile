...
jni/librust.a: jni/*.rs jni/libcgmath.rlib jni/libpng.rlib
	$(PRE_RUSTC) $(RUSTC) --target=arm-linux-androideabi jni/main.rs -C linker=$(ANDROID_NDK_STANDALONE_HOME)/bin/arm-linux-androideabi-gcc --crate-type=staticlib -C opt-level=3 -o jni/librust.a -L jni
...
jni/libcgmath.rlib: external/cgmath-rs/src/*.rs
	$(PRE_RUSTC) $(RUSTC) --target=arm-linux-androideabi external/cgmath-rs/src/cgmath.rs -C linker=$(ANDROID_NDK_STANDALONE_HOME)/bin/arm-linux-androideabi-gcc --crate-type=rlib -C opt-level=3 -o jni/libcgmath.rlib

jni/libpng.rlib: external/rust-png/*.rs jni/libpng.a jni/libshim.a
	$(PRE_RUSTC) $(RUSTC) --target=arm-linux-androideabi external/rust-png/lib.rs -C linker=$(ANDROID_NDK_STANDALONE_HOME)/bin/arm-linux-androideabi-gcc --crate-type=rlib -C opt-level=3 -o jni/libpng.rlib -L jni
